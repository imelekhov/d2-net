...
class MegaDepthSTDataset(Dataset):
    def __init__(
            self,
            base_path="/ssd/data/MegaDepth_full",
            train=True,
        ...
        self.preprocessing = init_transforms(color_aug)
        self.min_overlap_ratio = min_overlap_ratio
        self.max_overlap_ratio = max_overlap_ratio
        self.max_scale_ratio = max_scale_ratio
        self.image_size = image_size
        
        self.scene_info_path = osp.join(self.base_path, "scene_info")
        self.selected_ids_path = osp.join(self.base_path, "selected_ids")

        scene_list_path = "megadepth_utils/train_scenes.txt" if self.train else "megadepth_utils/valid_scenes.txt"
        self.scenes = []
    ...
    def build_dataset(self):
        ...
        for scene in self.scenes:
            if not osp.isfile(osp.join(self.selected_ids_path, scene + "_ids.pkl")):
                continue

            scene_info_path = osp.join(self.scene_info_path, scene + ".0.npz")
            ...
            with open(osp.join(self.selected_ids_path, scene + "_ids.pkl"), "rb") as f:
            ...
            for pair_idx in selected_ids:
                ...
                depth_fname1, depth_fname2 = depth_paths[idx1], depth_paths[idx2]

                try:
                    with h5py.File(osp.join(self.base_path, depth_fname1[depth_fname1.find("MegaDepth_v1"):]), 'r') as hdf5_file:
                        depth1 = np.array(hdf5_file['/depth'])
                    assert (np.min(depth1) >= 0)
                    image1 = Image.open(osp.join(self.base_path, image_paths[idx1]))
                    if image1.mode != 'RGB':
                    ...
                    print("bad tuple 1")
                    # print("Depth1 could not read. fname: ", depth_fname1[depth_fname1.find("MegaDepth_v1"):])
                    invalid_depth_files.append(depth_fname1[depth_fname1.find("MegaDepth_v1"):])
                    continue

                try:
                    with h5py.File(osp.join(self.base_path, depth_fname2[depth_fname2.find("MegaDepth_v1"):]), 'r') as hdf5_file:
                        depth2 = np.array(hdf5_file['/depth'])
                    assert (np.min(depth2) >= 0)
                    image2 = Image.open(osp.join(self.base_path, image_paths[idx2]))
                    if image2.mode != 'RGB':
                    ...
                    print("bad tuple 2")
                    # print("Depth2 could not read. fname: ", depth_fname2[depth_fname2.find("MegaDepth_v1"):])
                    invalid_depth_files.append(depth_fname2[depth_fname2.find("MegaDepth_v1"):])
                    continue

                self.dataset.append({
                    'image_path1': image_paths[idx1],
                    'depth_path1': depth_fname1[depth_fname1.find("MegaDepth_v1"):],
                    'intrinsics1': intrinsics[idx1],
                    'pose1': poses[idx1],
                    'image_path2': image_paths[idx2],
                    'depth_path2': depth_fname2[depth_fname2.find("MegaDepth_v1"):],
                    'intrinsics2': intrinsics[idx2],
                    'pose2': poses[idx2],
                    'central_match': central_match,
                    'scale_ratio': max(nd1 / nd2, nd2 / nd1)
                })
            print("Scene ", str(scene_id + 1), "/", str(len(self.scenes)))
            scene_id += 1
    ...
    def recover_pair(self, pair_metadata):
        ...
        try:
            ...
            print("Depth1 could not read. fname: ", depth_path1)
            sys.exit()
    ...
    def __getitem__(self, idx):
        ...
        '''
        image1 = self.preprocessing(image=image1)["image"]
